...
/**
 * TileMap. Holds the data of the nodes and tiles that make up the labyrinth. Connects the nodes and tiles logically. Responsible for
 * drawing the Tiles.
 *
 * @author frontend
 */
...
public class TileMap extends Actor {
	...
	/**Constructor. Generates a new Node matrix and sets the bounds of the map depending on the size of the labyrinth.
	 * 
	 * @param sizeX
	 * @param sizeY
	 */
	...
	/**
	 * Constructor. Builds TileMap with given matrix and sets bounds depending on the size of the labyrinth.
	 * @param matrix
	 */
	...
	/**
	 * Changes the Node at the coordinates of given Node with given node. Updates the Tile based on new Node.
	 * @param node
	 */
	public void updateMap(Node node) {
		...
		tiles[node.getVertIndex()][node.getHorIndex()] = buildTileBasedOnNodeType(node, tilePosition);
	}
