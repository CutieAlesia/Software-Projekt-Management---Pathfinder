...
public class RecursiveDivision implements LabyrinthGenerator {
     ...
     *     <p>Runs until a wall, the end node, the start node or a passage is found. Then a new wall
     *     is drawn with a randomly generated passage index. After the work is done it recursively
     *     calls drawWallToLeft and drawWallToRight to generate new walls with the previously
     *     generated wall as their origin
     */
    private void drawWallToTop(int x, int y) {
        ...
        for (int i = y - 1; i >= 0; i--) {
            ...
            boolean rightIsWall =
                    x + 1 < matrix.length && matrix[x + 1][i].getType() == NodeType.BLOCKED;
        ...
        if (spaces >= 2) {
            ...
            if (spaces >= 5) idx = randomNumber(y - 1, y - spaces);

            if (lastIsPassage) idx = y - spaces;
    ...
    private void drawWallToBottom(int x, int y) {
        ...
        for (int i = y + 1; i < matrix[0].length; i++) {
            ...
            if (type == NodeType.BLOCKED || type == NodeType.END || type == NodeType.START) break;
     ...
     *     <p>Runs until a wall, the end node, the start node or a passage is found. Then a new wall
     *     is drawn with a randomly generated passage index. After the work is done it recursively
     *     calls drawWallToTop and drawWallToBottom to generate new walls with the previously
     *     generated wall as their origin
     */
    private void drawWallToLeft(int x, int y) {
        ...
        for (int i = x - 1; i >= 0; i--) {
            ...
            boolean rightIsWall =
                    y + 1 < matrix[0].length && matrix[i][y + 1].getType() == NodeType.BLOCKED;
        ...
        if (spaces >= 2) {
            ...
            if (spaces >= 5) idx = randomNumber(x - 1, x - spaces);

            if (lastIsPassage) idx = x - spaces;
    ...
    /**
     * Randomly generates a new wall index, based on a min and max value and the generated passage
     * value
     *
